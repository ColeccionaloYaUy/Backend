...
public static class PaginationHelper {
	...
	public static async Task<PagedData<T>> FetchPagedAsync<T>(
		...
		var (items, totalCount) = await RunPage<T>(connection, sql, applyParams, map, page, pageSize, cancellationToken);

		if (items.Count == 0 && page > 1) {
			(items, totalCount) = await RunPage<T>(connection, sql, applyParams, map, 1, pageSize, cancellationToken);
