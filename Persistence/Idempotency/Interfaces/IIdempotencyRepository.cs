...
namespace ColeccionaloYa.Persistence.Idempotency.Interfaces {
    public interface IIdempotencyRepository {
        ...
        /// </summary>
        Task<IdempotencyKey?> TryInsertAsync(string key, int? clientId, string endpoint, string requestHash);

        /// <summary>
        /// Marca la key como 'completed' y guarda el resultado.
        /// </summary>
        Task CompleteAsync(string key, int? clientId, int statusCode, string responseBody);

        /// <summary>
        /// Marca la key como 'failed' (para que el cliente pueda reintentar).
        /// </summary>
        Task FailAsync(string key, int? clientId);
    }
}
